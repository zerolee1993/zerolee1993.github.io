...
这个链接中有我整理的一些教程，建议你将python和numpy的部分看完，或者自己找资料了解python和numpy
http://www.lishaofei.com/tags.html
...
### 12. Python中的广播(boradcasting in python)

之前提到的 $Z=w^T x+b$，python会将标量b转换成(1,m)的矩阵 $[b,b,...,b]$ ，再运行，这就是python中的广播机制

类似的情况有很多，如下面的操作数1，操作数2，进行加减乘除时，会将操作数2进行转化，表中展示的是shape
...
使用python的numpy时一定注意，不要使用秩为1的数组，比如`a=np.random.randn(5)`此时a.shape=(5,)

这样会导致不可预知的问题

尽量用`a=np.random.randn((5,1))`或`a=np.random.randn((1,5))`

或者使用reshape改变数组的shape
